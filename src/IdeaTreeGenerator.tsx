...
export default function IdeaTreeGenerator() {
  ...
  const [editingDescriptionNodeId, setEditingDescriptionNodeId] = useState(null);
  ...
  const calculateSemanticScores = (eventType, actor, eventData) => {
    ...
    switch (eventType) {
      case 'Initial Input':
        // Initial Input is a significant human contribution - starts at Human 100%, Generative Expansion 100%
        scores.semantic_generative_score = 3; // Maximum generative expansion (human input starts the creative process)
...

{
  ...
  const handleEditDescription = (node) => {
    setEditingDescriptionNodeId(node.id);
    setEditDescriptionValue(node.description || node.text || '');
  };
  ...
  const handleSaveDescription = () => {
    ...
    trackEvent('Edit Node', 'HUMAN', {
      node_id: editingDescriptionNodeId.toString(),
      raw_content: nodeToEdit.description || nodeToEdit.text,
      new_content: editDescriptionValue
    });
...
- "advice" (optional): Must be only ONE sentence under 18 words, and must be a brief strategy-oriented suggestion that deepens or expands the idea. Example advice patterns: creative reframing strategies, gamification patterns, exploring cross-modal cues. Do not include solutions or implementation details. If no meaningful advice applies, omit this field.`
  ...
  if (mode === 'structure' && currentPage !== 'tracking') {
    ...
    return (
      <div className="w-full h-screen flex flex-col" style={{ background: 'linear-gradient(to bottom right, #FAF5FF, #FDF2F8, #FFF7ED)' }}>
          ...
          <div className="absolute top-6 left-[calc(50%-192px)] transform -translate-x-1/2 z-50">
            <div className="bg-white shadow-lg border border-purple-200 px-2 py-2 flex items-center gap-1" style={{ borderRadius: '16px' }}>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-purple-50 transition-colors text-gray-700 font-medium"
                title="Back to Exploration Mode"
              >
                <ArrowLeft size={18} />
    ...
    >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {mode === 'exploration' && nodes.length > 0 && (
          <div className={`absolute top-6 z-50 ${isReflectionSidebarOpen ? 'left-[calc(50%-200px)]' : 'left-1/2'} transform -translate-x-1/2`}>
            ...
            <div className="bg-white rounded-lg shadow-lg border border-purple-200 px-2 py-2 flex items-center gap-1">
              ...
              {/* Structure Mode Button - Unified: Navigate to existing or generate new */}
              <button
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  ...
                  // If structure exists and no nodes selected, just navigate
                  if (hierarchyAnalysis && selectedForStructure.size < 2) {
                    setMode('structure');
                  }
                  // If nodes are selected (2+), generate new structure (even if one exists)
                  else if (selectedForStructure.size >= 2) {
                    analyzeHierarchy();
                  }
                }}
                disabled={
                  analyzingStructure ||
                  (!hierarchyAnalysis && selectedForStructure.size < 2)
                }
                className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all text-gray-700 font-medium ${analyzingStructure
                  ...
                  analyzingStructure
                    ...
                    : hierarchyAnalysis && selectedForStructure.size < 2
                      ...
                      : selectedForStructure.size >= 2
                        ? `Generate Structure (${selectedForStructure.size} selected)`
                        : 'Select at least 2 nodes to generate structure'
                }
              >
                {analyzingStructure ? (
                  <>
                              ...
                              <div
                                ...
                                style={{
                                  ...
                                  boxShadow: isSelectedForStructure && selectedForStructure.size > 1
                                    ...
                                    : (isSelected && nodeCreationType === 'human')
                                      ? '0 4px 12px rgba(152, 16, 250, 0.3), 0 2px 6px rgba(152, 16, 250, 0.2)'
                                      : (isSelected && nodeCreationType === 'ai')
                                        ? '0 4px 12px rgba(254, 98, 21, 0.3), 0 2px 6px rgba(254, 98, 21, 0.2)'
                                        : (isSelected && nodeCreationType === 'co-creation')
                                          ? '0 4px 12px rgba(173, 70, 255, 0.3), 0 2px 6px rgba(246, 51, 154, 0.2)'
                                          : undefined
                                }}
                              >
                  ...
                  {editingDescriptionNodeId === selectedNodeData.id ? (
                    <div>
                      <textarea
                        ...
                        className="w-full p-3 border border-gray-300 rounded-lg text-gray-700 leading-relaxed resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
                        rows={6}
                        autoFocus
                      />
        ...
        {!isReflectionSidebarOpen && reflections.length > 0 && (
          ...
          >
            <Lightbulb className="text-[#AD46FF]" size={28} strokeWidth={2.5} style={{ stroke: '#AD46FF', fill: '#AD46FF' }} />
          </button>
        )}
